...
# ╔═╡ c52d8bf4-d01e-40cc-a0d9-1daa4f018bf8
begin
...
# ╔═╡ bfd81ed9-94ad-4aaa-8b3f-e77c8df831bb
md"""
# Double Quantum Dot in a cavity

D. Zenelaj, P. P. Potts, and P. Samuelsson, Full counting statistics of the photocurrent through a double quantum dot embedded in a driven microwave resonator, *Phys. Rev. B* **106**, 205135 (2022).
"""

# ╔═╡ 9c6ebffe-a8d5-45c5-ba36-b05011822a44
md"""
### Some useful functions
"""

# ╔═╡ 85402fda-ec06-11f0-9146-7f53c98c9b16
begin
	fermi = (ϵ, μ, T) -> 1/(1 + exp((ϵ - μ)/T))
	
	# Lindblad dissipator
	D = (X, ρ) -> X*ρ*X' - 1/2*X'*X*ρ - 1/2*ρ*X'*X
end

# ╔═╡ c45b50f6-0efd-4784-9c5b-0d6d10acff3e
md"""
### Parameters from Fig.3
Convention $\hbar = k_B = e =1$
"""

# ╔═╡ b3493287-8df8-4a74-8dd5-13314838dc80
md"""
#### DQD-Leads parameters
"""
...
"""
function get_Ω(Δϵ::Real, tc::Real)
...
"""
function get_θ(Δϵ::Real, tc::Real)
	θ = acos(- Δϵ / get_Ω(Δϵ, tc))
	return θ
end

# ╔═╡ bbd22626-6ca7-449b-85cc-6d9be4f4ee99
"""
DQD eigen-energies
"""
function get_eigen_energies(Δϵ::Real, tc::Real)
	# TODO: Add ϵ_avg to the eigen_energies
	Ω = get_Ω(Δϵ, tc)
	ϵg = - Ω / 2.
	ϵe = Ω / 2.
	return ϵg, ϵe
...
# ╔═╡ e26dadd8-f891-414d-ab28-fd02d85698ce
"""
DQD-Leads transition rates in the ground-excited basis
"""
function get_Γge(
	Δϵ::Real, tc::Real,
	TL::Real, TR::Real,
	...
	ϵg, ϵe = get_eigen_energies(Δϵ, tc)
	θ = get_θ(Δϵ, tc)
...
# ╔═╡ 5e1bc932-2958-4bcf-8347-30f28331cb67
"""
Thermal occupation at the cavity frequency
"""
function get_nth_cav(
	Δϵ::Real, tc::Real,
	Tcav::Real,
	Δd::Real, Δr::Real
)
	...
	nth_cav = 1. / (exp(ωr/Tcav) - 1.);
	return nth_cav
end

# ╔═╡ 6637f3a4-a18a-4012-a48e-8d5afbbd9d58
"""
Displacement field
"""
function get_α(
	...
	α = (-2im * sqrt(κin * Ndot)/(κ + 2im * Δr));
	return α
end

# ╔═╡ 4ce9a224-bd08-418b-a05b-bf104da34afd
"""
Steady-state photton number in the cavity without DQD
"""
function n_cav_ss(
	Δr::Real, κin::Real, Ndot::Real, κ::Real 
)
	abs(2. * sqrt(κin * Ndot)/(κ + 2im * Δr))^2
end

# ╔═╡ 2b488c24-8542-4914-8bd3-55ad9401e783
md"""
### Operators
"""

# ╔═╡ 3e790484-fb87-497a-b4cc-36477b6d25c1
md"""
#### DQD operators
"""

# ╔═╡ 2190a78c-f37f-4bbe-807c-729dd89a3429
begin
	# Basis
	ket_0, ket_L, ket_R = basis(3, 0), basis(3,1), basis(3, 2);
	
	# Number operators
	nL_loc = ket_L * ket_L';
	nR_loc = ket_R * ket_R';

	# Allowed transitions in the Coulomb blockade regime
	sL_loc = ket_0 * ket_L';
	sR_loc = ket_0 * ket_R';
...
# ╔═╡ bbd67175-6f69-44b3-a319-1a1c1be1b3e9
md"""
#### Cavity operators
"""

# ╔═╡ dbfafe7c-2785-451c-8bf7-4b7dcdcae188
begin
	Ncav = 3;
	id_Cavity = qeye(Ncav);
	a_loc = destroy(Ncav);
...
# ╔═╡ a67b79ee-3a73-42fe-ae69-b29fd0089b52
md"""
#### DQD-Cavity operators
"""

# ╔═╡ 52da6eaf-92fb-4c81-ac00-91f2e1177344
begin
	# Annihilation operator
	a = tensor(id_Dqd, a_loc);

	# Left-right basis
	nL = tensor(nL_loc, id_Cavity);
	nR = tensor(nR_loc, id_Cavity);
	sL = tensor(sL_loc, id_Cavity);
	sR = tensor(sR_loc, id_Cavity);
end

# ╔═╡ 0cd6ffbb-a22a-4aa1-bbb8-16e0495c8ac4
md"""
#### Derived operators
"""

# ╔═╡ 6fa9378b-84a2-48a5-b4a9-cb5bad821c2f
"""
DQD-Cavity ladder operators in the ground-excited basis
"""
function get_sge(Δϵ::Real, tc::Real)
	θ = get_θ(Δϵ, tc);
...
# ╔═╡ 004e1dd6-846f-4a1f-9d80-8a7e80cf30c4
"""
σz operator in the ground-excited basis
"""
function get_σz_ge(Δϵ::Real, tc::Real)
	sg, se = get_sge(Δϵ, tc);
...
# ╔═╡ d04532b4-9e9a-4fda-8cf1-be9c09e0f321

"""
Raising and lowering operators for the DQD
"""
function get_σmp(Δϵ::Real, tc::Real)
	sg, se = get_sge(Δϵ, tc);
...
# ╔═╡ fe6a6be1-3c17-436d-8d86-da9f92a22121
md"""
## Currents
"""

# ╔═╡ cd9c7422-b83e-4498-9eee-1be84b996f4b
"H and c_ops for DQD + cavity (displaced field)"
function H_cops_2qdot_cav_disp(
	Δϵ::Real, tc::Real,
	TL::Real, TR::Real,
	μL::Real, μR::Real,
	ΓL::Real, ΓR::Real,
	κin::Real, Ndot::Real, Tcav::Real,
	κ::Real, gJC::Real,
    Δd::Real, Δr::Real,
	γm::Real, γphi::Real
)
	# Derived parameters
	α = get_α(κin, Ndot, κ, Δr)
	Γg0, Γe0, Γ0g, Γ0e = get_Γge(
		Δϵ, tc,
		TL, TR,
		μL, μR,
		ΓL, ΓR;
	...
	nth_cav = get_nth_cav(Δϵ, tc, Tcav, Δd, Δr)
	# Derived operators
	σz = get_σz_ge(Δϵ, tc)
	σm, σp = get_σmp(Δϵ, tc)
	sg, se = get_sge(Δϵ, tc)
    # Hamiltonian
	Htot = (Δd/2.) * σz + Δr * (a + α)' * (a + α) + gJC * ((a + α)' * σm + (a + α) * σp) + sqrt(κin * Ndot) * ((a + α)' + (a + α));
     
	# Jump operators
	c_ops = [
		sqrt(Γg0) * sg', sqrt(Γ0g) * sg, sqrt(Γ0e) * se, sqrt(Γe0) * se',
		sqrt(γphi)/2 * σz, sqrt(γm) * σm,
		sqrt(κ * (nth_cav + 1)) * (a + α), sqrt(κ * nth_cav) * (a + α)'
	];

    return Htot, c_ops
end

# ╔═╡ b7c62a0d-1a08-4523-9ac6-8d8304784564
begin
	"current from the left contact for DQD + cavity"
	function current_L_2dot_cav(
		ρ::QuantumObject,
		Δϵ::Real, tc::Real,
		TL::Real, TR::Real, μL::Real, μR::Real,
		ΓL::Real, ΓR::Real
	)
		# Derived parameters
		Γg0L, Γe0L, Γ0gL, Γ0eL = get_Γge(
			Δϵ, tc,
			TL, TR, μL, μR,
			ΓL, ΓR;
			side = "left"
		...
		sg, se = get_sge(Δϵ, tc)
		# Dissipator
	    Lrho = Γg0L * D(sg', ρ) + Γ0gL * D(sg, ρ) + Γe0L * D(se', ρ) + Γ0eL * D(se, ρ);

	    return real(expect(nL + nR, Lrho))
	end
end

# ╔═╡ 82f72f10-585b-48da-aafd-058267cf49cc
"H and c_ops for DQD + cavity (displaced field) at zero temperature and large drive limit"
function H_cops_2qdot_cav_large_drive(
	Δϵ::Real, tc::Real,
	κin::Real, Ndot::Real,
	κ::Real, Δd::Real, Δr::Real,
	γm::Real, γphi::Real
)
	# Derived parameters
	α = get_α(κin, Ndot, κ, Δr)
	Γg0, Γe0, Γ0g, Γ0e = get_Γge(Δϵ, tc, TL, TR, μL, μR, ΓL, ΓR)
	...
	# Hamiltonian
	Htot = (Δd/2.) * σz + gJC * (conj(α) * σm + α * σp);

	# Jump operators
	c_ops = [
		sqrt(Γg0) * sg',  sqrt(Γ0e) * se,
		(sqrt(γphi)/2.) * σz, sqrt(γm) * σm
	];
	return Htot, c_ops
end

# ╔═╡ 852af287-8123-4e83-8f75-28fad41ffaac
"current from the left contact for DQD + cavity at zero temperature and larg drive limit"
function current_L_2dot_cav_large_drive(
	ρ::QuantumObject,
	Δϵ::Real, tc::Real,
	TL::Real, TR::Real, μL::Real, μR::Real,
	ΓL::Real, ΓR::Real
	...
	sg, se = get_sge(Δϵ, tc)
	# Dissipator
	Lrho = Γg0L * D(sg', ρ) + Γ0eL * D(se, ρ);

	return real(expect(nL + nR, Lrho))
end

# ╔═╡ fdbbc77d-4f8f-452f-9115-8f2309b49ee7
"""
Current in the large drive limit and the zero temperature limit
"""
function I_large_drive_lim(Δϵ::Real, tc::Real)
    Ω = get_Ω(Δϵ, tc);
    θ = get_θ(Δϵ, tc);
    ΓLin = ΓL*cos(θ/2)^2;
    ΓRin = ΓR*sin(θ/2)^2;
    ...
    (ΓLin * ΓRout - ΓRin * ΓLout)/(Γ0e + 2. * Γg0)
end

# ╔═╡ 4d057822-f923-4925-ba8c-dd9e6edeba46
md"""
### Plots
"""

# ╔═╡ 38931408-6738-49eb-9408-53d1abb6893b
let
	Ndots = range(0, 100, 200);
	ss_disp = [steadystate(
		H_cops_2qdot_cav_disp(
			Δϵ, tc,
			TL, TR,
			μL, μR,
			ΓL, ΓR,
	...
	ss_large_drive = steadystate(
		H_cops_2qdot_cav_large_drive(
			Δϵ, tc,
			κin, 5000,
			κ, Δd, Δr,
			γm, γphi)...);
	IL = [current_L_2dot_cav_large_drive(
			ss_disp[i],
			Δϵ, tc,
			TL, TR, μL, μR,
			ΓL, ΓR
		) for i in 1:length(Ndots)];
	IL_large_drive = current_L_2dot_cav_large_drive(
		    ss_large_drive,
		    Δϵ, tc,
		    TL, TR, μL, μR,
		    ΓL, ΓR
		)
	IL_large_drive_analytics = I_large_drive_lim(Δϵ, tc)
	fig = Figure()
	ax = Axis(fig[1, 1], xlabel="Ndot", ylabel="I")
	lines!(ax, Ndots, IL)
	lines!(ax, [0, last(Ndots)], [IL_large_drive, IL_large_drive], linestyle=:dash)
	lines!(ax, [0, last(Ndots)], [IL_large_drive_analytics, IL_large_drive_analytics], linestyle=(:dot, 4), color=:red, linewidth=3)
	fig
end

# ╔═╡ 5279bd18-3feb-4219-a033-39d4fcea59ba
let
	Ndots = range(0, 25, 1000);
	ss_disp = [steadystate(H_cops_2qdot_cav_disp(
		Δϵ, tc,
		TL, TR,
		μL, μR,
		ΓL, ΓR,
	...
	IL = [current_L_2dot_cav(
		    ss_disp[i],
		    Δϵ, tc,
		    TL, TR, μL, μR,
		    ΓL, ΓR
		) for i in 1:length(Ndots)];
	Ilim = I_large_drive_lim(Δϵ, tc)
	
	fig = Figure()
	ax = Axis(fig[1, 1], xlabel="f=sqrt(κin Ndot)", ylabel="I")
	lines!(ax, sqrt.(κin*Ndots), IL)
	lines!(ax, [0, sqrt(κin*last(Ndots))], [Ilim, Ilim])
	fig
end

# ╔═╡ a4d69f3b-621f-466e-b60c-643e1bfe2336
let
	Ndots = range(0, 25, 1000);
	ss_disp = [steadystate(H_cops_2qdot_cav_disp(
		Δϵ, tc,
	...
	α_list = [get_α(κin, ndot, κ, Δr) for ndot in Ndots];
	nsscav = [expect((a + α_list[i])' * (a + α_list[i]), ss_disp[i]) for i in 1:length(Ndots)];
	nss0 = [n_cav_ss(Δr, κin, ndot, κ) for ndot in Ndots];

	fig = Figure()
	ax = Axis(fig[1, 1], xlabel="T", ylabel="I")
	
	lines!(ax, Ndots, real.(nsscav), label="with DQD")
	lines!(ax, Ndots, nss0, label="no DQD")
...
manifest_format = "2.0"
project_hash = "0f03d5224437c6ca5d601d9c011305f81c67a3e4"
...
[[deps.QuantumToolbox]]
deps = ["ArrayInterface", "DiffEqBase", "DiffEqCallbacks", "DiffEqNoiseProcess", "Distributed", "FFTW", "Graphs", "IncompleteLU", "LaTeXStrings", "LinearAlgebra", "LinearSolve", "OrdinaryDiffEqCore", "OrdinaryDiffEqTsit5", "Pkg", "Random", "SciMLBase", "SciMLOperators", "SparseArrays", "SpecialFunctions", "StaticArraysCore", "StochasticDiffEq"]
git-tree-sha1 = "11ea14eed43d889a3bca81cf5b972ebe43eeee37"
